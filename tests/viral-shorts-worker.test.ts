...
test("stage 3 draft render-plan override strips channel-managed template fields", () => {
  ...
  assert.deepEqual(sanitizeStage3DraftRenderPlanOverride(rawOverride), {
    ...
    focusX: base.focusX,
    videoZoom: 1.4,
    mediaRegionHeightPx: base.mediaRegionHeightPx,
    videoScaleY: base.videoScaleY,
    videoScaleX: base.videoScaleX,
    videoFit: base.videoFit,
...
test("normalizeChatDraft removes legacy template id from stage 3 render-plan override", () => {
  ...
  assert.deepEqual(draft?.stage3.renderPlan, {
    ...
    videoZoom: 1.2,
    mediaRegionHeightPx: fallbackRenderPlan().mediaRegionHeightPx,
    videoScaleY: fallbackRenderPlan().videoScaleY,
    videoScaleX: fallbackRenderPlan().videoScaleX,
    videoFit: fallbackRenderPlan().videoFit,
    sourceCrop: fallbackRenderPlan().sourceCrop,
...
test("step 1 renders a custom mp4 upload panel and auto-stage2 checkbox", () => {
  ...
  assert.match(html, /Время публикации YouTube/);
  assert.match(html, /type="datetime-local"/);
  assert.match(html, /tag1, tag2, tag3/);
  assert.match(html, /ближайший свободный слот автоматически/);
