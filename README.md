...
- Stage 2:
  ...
  - workspace owner может переключить eligible caption-only stages на Anthropic API или OpenRouter API:
    ...
    - Stage 2 prompt/examples теперь могут иметь format profiles: default profile сохраняет старое поведение, а `Channel + Story` profile включается только явно и пишет Lead/Body поверх прежнего `top` / `bottom` handoff;
