...
- Для внешнего участника, который только подключает YouTube, owner/manager создаёт готовый аккаунт `channel_connector` на `/team`: выбирает один или несколько каналов, а система один раз показывает логин, сгенерированный пароль, ссылку `/connect/login` и готовую пересылаемую инструкцию. Регистрация и accept-invite не нужны.
- `channel_connector` использует отдельную connector-session и портал `/connect`; обычная Clips session для этой роли не создаётся. Разрешено только подключение Google и выбор YouTube destination у каналов с explicit `connect` grant.
...
- `redactor_limited` остаётся отдельной явной ролью для случаев, когда нужно оставить доступ к работе с каналом без права менять полный setup.
- `redactor_limited` с выданным channel access может открыть Channel Manager только в ограниченном режиме `Рендер`/`Ассеты`: загрузить фон/музыку, импортировать backup шаблона в настройки рендера канала, выбрать дефолтный фон/музыку и хронометраж. Бренд, Stage 2, Publishing setup, YouTube/OAuth, удаление ассетов, доступы и создание/удаление каналов остаются закрыты.
- В Step 3 `redactor_limited` с выданным channel access также может загружать фон/музыку, импортировать backup шаблона в текущий draft и менять время публикации.
...
- Stage 2:
  ...
  - owner observability is workspace-level:
    ...
    - approved MacBook/Mac mini agent owner access uses `npm run mcp:owner` plus a full-scope machine credential from `/api/admin/mcp-machines`; this avoids recurring short-lived token creation while keeping revoke/audit/rotation visibility;
    - machine health checks live in `npm run macmini:healthcheck`; persistent Clips MCP plus Git/Render deployment setup is documented in [`docs/macmini-owner-mcp.md`](/Users/neich/Documents/Macedonian Imperium/clips automations/docs/macmini-owner-mcp.md);
...
- `template-road` теперь считает card geometry и межстрочный интервал частью настоящего render contract:
  ...
  - Step 3 позволяет отдельно менять вертикальный масштаб исходного видео, чтобы сплющивать или растягивать только source slot без изменения текста, карточки или фона.
  - Step 3 позволяет менять длину готового ролика для конкретного draft, не меняя дефолт канала.
  - Step 3 позволяет отдельно регулировать громкость звука исходника и вручную добавлять highlight-spans к выбранным словам финального текста через палитру активных цветов шаблона.
