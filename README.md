...
- `APP_STORAGE_RENDER_EXPORT_MAX_AGE_HOURS` / `APP_STORAGE_RENDER_EXPORT_EMERGENCY_MAX_AGE_HOURS` — сколько хранить неактивные render exports. Defaults: `168` часов в обычном режиме, `24` часа в аварийном. Файлы queued/uploading/paused/failed и scheduled без `youtube_video_id` защищены. После того как YouTube вернул непустой video id, локальный render и upload-source освобождаются ближайшей фоновой уборкой независимо от возраста; активная задача, использующая тот же source, сохраняет hard protection.
