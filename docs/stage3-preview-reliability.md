...
- Если queued local job осталась после watchdog timeout, но у текущего пользователя больше нет running local job и свежего online worker-а, статусный poll должен завершить её recoverable `worker_unavailable`, а не оставлять UI в бесконечном `Ожидает локальный executor`.
- Host executor использует тот же kind-specific timeout guard для active jobs. Если hosted `editing-proxy` / `preview` / `render` завис и держит единственный CPU slot, runtime помечает job recoverable `*_timeout`, отправляет abort-сигнал и запускает следующий queued job.
