...
Stage 2 для `Channel + Story` должен также отдавать render-safe body window: `mainCaption` ограничивается более коротким body-окном, чем классический bottom, чтобы 6-секундный Hollywood/story layout не получал плотный абзац, который затем приходится спасать резким уменьшением шрифта в Step 3.
