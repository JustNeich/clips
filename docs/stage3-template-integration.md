...
## 9a. Custom font asset contract

Template Road может подключать пользовательские font files отдельно для двух wire-слотов:
- `top` = `TOP` в классической семье и `lead` в `Channel + Story`;
- `bottom` = `BOTTOM` в классической семье и `body / main caption` в `Channel + Story`.

Правила:
- разрешены только `TTF`, `OTF`, `WOFF` и `WOFF2`;
