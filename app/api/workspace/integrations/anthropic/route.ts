...
import {
  ...
  mutateWorkspaceAnthropicIntegration
...
export async function GET(): Promise<Response> {
  try {
    ...
    const integration = await getWorkspaceAnthropicStatus(auth);
    return Response.json({ integration }, { status: 200 });
  } catch (error) {
    return asErrorResponse(error, "Не удалось загрузить Anthropic integration.", 403);
  }
}
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    return asErrorResponse(error, "Не удалось обновить Anthropic integration.", 403);
  }
}
